...
- `GEMINI_REVIEW_MAX_CONTENT_LENGTH`: max MCP request body size in bytes, defaults to `52428800`
...
- The server auto-loads only `GEMINI_API_KEY` and `GOOGLE_API_KEY` from `~/.gemini/.env` for direct API calls; the file is ignored if it is a symlink, not owned by the current user, not a regular file, or group/world-writable. Keys loaded this way are not written to process-global `os.environ` and are not implicitly exported to CLI or `agy` subprocesses.
